...
export class OrganizationsService {
      ...
      include: {
        _count: {
          select: {
            ...
            events: { where: { status: { in: ['PUBLISHED', 'ONGOING'] } } },
          },
        },
      },
    });
      ...
      include: {
        user: {
          select: {
            ...
            verificationLevel: true, averageRating: true,
          },
        },
      },
    });
  ...
  async inviteMember(orgId: string, inviterId: string, dto: InviteMemberDto) {
    ...
    this.notifications
      .notify(
        user.id,
        NotificationType.APPLICATION_RECEIVED, // closest type — could add ORG_INVITE later
        'Organization Invitation',
        `You have been invited to join ${org?.name ?? 'an organization'} as ${dto.role.replace(/_/g, ' ')}`,
        { orgId },
      )
      .catch(() => {});
